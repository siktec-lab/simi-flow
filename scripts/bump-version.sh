...
echo ""
echo "Note: 'main' is protected (PR-only, owner-only tags). Prefer the"
echo "release helper, which opens the PR and tags for you:"
echo ""
echo "  bash scripts/release.sh pr  $NEW_VERSION   # branch + bump + open PR"
echo "  # ...after the PR is merged and CI is green:"
echo "  bash scripts/release.sh tag $NEW_VERSION   # tag main -> publish"
